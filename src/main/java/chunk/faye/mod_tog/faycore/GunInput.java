...
public class GunInput {
   ...
   private static void fireGun(Minecraft mc) {
      ...
      if (SkillInteractScreen.GunMode != 2) {
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc,
            "execute as %player% at @s positioned ^ ^-0.25 ^1 run particle minecraft:dust{color:[0.2,0.2,0.2],scale:0.5} ~ ~1.75 ~ 0.05 0.05 0.05 0 10 force @a"
         );
      } else {
         Vec3 target = getTargetLocation(mc, 100.0);
         double ex = target.x;
         double ey = target.y;
         double ez = target.z;
         ...
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc,
            "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run particle minecraft:dust{color:[0.2,0.2,0.2],scale:0.5} ~ ~1.75 ~ 0.05 0.05 0.05 0 10 force @a"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run particle minecraft:lava ~ ~1.75 ~ 0.05 0.05 0.05 0 10 force @a"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run particle minecraft:explosion ~ ~1.75 ~ 0.05 0.05 0.05 1 1 force @a"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run playsound minecraft:entity.dragon_fireball.explode player @a ~ ~ ~ 1 1 1"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run playsound minecraft:entity.generic.explode player @a ~ ~ ~ 1 1 1"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run playsound minecraft:item.trident.thunder player @a ~ ~ ~ 1 1 1"
         );
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, "execute as %player% at @s positioned ^-0.25 ^-0.25 ^1.5 run playsound minecraft:entity.wither.shoot player @a ~ ~ ~ 1 2 1"
         );
      }

      if (SkillInteractScreen.GunMode == 0) {
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc, String.format("execute positioned %f %f %f run particle minecraft:lava ~ ~ ~ 0 0 0 0.25 10 force @a", targetPos.x, targetPos.y, targetPos.z)
         );
      } else if (SkillInteractScreen.GunMode == 1) {
         FayCoreMacroEngine.autoFindAndInjectVCommand(
            mc,
            String.format(
               "execute positioned %f %f %f run particle minecraft:raid_omen ~ ~ ~ 0.25 0.25 0.25 1.245415898 25 force @a",
               targetPos.x,
               targetPos.y,
               targetPos.z
            )
      ...
      if (entityHit != null) {
         ...
         if (SkillInteractScreen.GunMode == 0) {
            ...
            String customDeathMessageCmd = String.format(
               "execute as %s if entity @s[type=minecraft:player] if entity @s[nbt={Health:0f}] run tellraw @a [\"\",{selector:\"%s\"},\" was killed by %%player%% using \",\"[\",{text:\"§9%%player%%'s Gun\",hover_event:{action:\"show_text\",value:[\"§9FayeCruz's Gun\\n\",{\"text\":\"ꜰᴀʏᴄᴏʀᴇ\",\"color\":\"#B0FF95\",italic:false},\"\\n§7Made by §cFayeCruz\"]}},\"]\"]",
               uuid,
               uuid
            );
            ...
            FayCoreMacroEngine.autoFindAndInjectVCommand(mc, gameruleCmd1);
            String customDeathMessageCmd = String.format(
               "execute as %s if entity @s[type=minecraft:player] unless entity @s[nbt={Health:0f}] run tellraw @a [\"\",{selector:\"%s\"},\" was killed by %%player%% using \",\"[\",{text:\"§9%%player%%'s Gun\",hover_event:{action:\"show_text\",value:[\"§9FayeCruz's Gun\\n\",{\"text\":\"ꜰᴀʏᴄᴏʀᴇ\",\"color\":\"#B0FF95\",italic:false},\"\\n§7Made by §cFayeCruz\"]}},\"]\"]",
               uuid,
               uuid
            );
            FayCoreMacroEngine.autoFindAndInjectVCommand(mc, customDeathMessageCmd);
