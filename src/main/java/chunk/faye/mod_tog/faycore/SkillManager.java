...
public class SkillManager {
    ...
    public static void castPressSkill(int slot) {
        ...
        switch (slot) {
            case 0 -> {
                ...
                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVPressCommand(mc, cmd1);
                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVPressCommand(mc, cmd2);
                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVPressCommand(mc, cmd3);
                ...
                java.util.concurrent.CompletableFuture.delayedExecutor(150, java.util.concurrent.TimeUnit.MILLISECONDS)
                        .execute(() -> mc.execute(() -> {
                            if (mc.player != null && mc.level != null) {
                                ...
                                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVReleaseCommand(mc, releaseCmd);
    ...
    private static void executeSingleCommandList(Minecraft mc, List<String> commands, boolean isPress) {
        for (String cmd : commands) {
            ...
            if (isPress) {
                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVPressCommand(mc, parsed);
            } else {
                chunk.faye.mod_tog.faycore.client.gui.FayCoreMacroEngine.executeVReleaseCommand(mc, parsed);
            }
        }
    }
